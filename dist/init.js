...
async function promptMasked(rl, question) {
  return new Promise((resolve) => {
...
function generateAgentName(address) {
  const suffix = address.slice(-8).toLowerCase();
  return `agent-${suffix}`;
}
async function createSignature(privateKey, walletAddress) {
  const wallet = new Wallet(privateKey);
  const timestamp = Date.now();
  const message = `Register agent: ${walletAddress} at timestamp: ${timestamp}`;
  const signature = await wallet.signMessage(message);
  return signature;
}
async function registerWithBackend(config, signature) {
...
function createAgentFiles(botDir, agentName, config) {
...
function updateMainConfig(botDir, agentName, config) {
  const configPath = join(botDir, "config.json");
  const handlerRiskProfile = config.riskProfile === "risk_averse" ? "conservative" : config.riskProfile === "risk_seeking" ? "aggressive" : "balanced";
  const mainConfig = {
    agent: {
      walletAddress: config.walletAddress,
      capital: config.capital,
...
async function selectOption(rl, prompt2, options) {
  console.log(`
${prompt2}`);
...
`);
  console.log(`This wizard will configure a new agent in 4 questions.
`);
  try {
    console.log("Question 1 of 4: Private Key");
    let privateKey;
    let walletAddress;
    while (true) {
...
Capital: ${formatUSD(capital)} USDC`);
    ...
    console.log(`
Question 3 of 4: Bet Sizing Strategy`);
    ...
    const betSizingKey = Object.keys(BET_SIZING_OPTIONS)[betSizingChoice];
    const betSizing = BET_SIZING_OPTIONS[betSizingKey];
    console.log(`
Bet size: ${betSizing.min}-${betSizing.max}% per bet`);
    console.log(`
Question 4 of 4: Risk Profile`);
    ...
    const riskProfile = Object.keys(RISK_PROFILE_OPTIONS)[riskChoice];
    console.log(`
...
`);
    ...
    createAgentFiles(botDir, agentName, config);
    ...
    const signature = await createSignature(privateKey, walletAddress);
    const registration = await registerWithBackend(config, signature);
